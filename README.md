...
### 2. Attention mechanism optimizations [Multi-Head, Multi-Query, Grouped-Query Attention, FlashAttention] — From Scratch Comparison

**File:** `distil_bert_pt_implementation.ipynb`

A from-scratch PyTorch implementation and side-by-side comparison of three attention variants — Multi-Head Attention (MHA), Multi-Query Attention (MQA), and Grouped-Query Attention (GQA) — trained under identical conditions to study their downstream performance.
...
- Flash Attention — Instead of the whole matrix, just tiny blocks are extracted from the giant matrix
...
A from-scratch PyTorch implementation and side-by-side comparison of two normalization techniques at architecture level, Pre-LN (Pre Layer Normalization) and Post-LN (Post Layer Normalization) — trained under identical conditions to study their impact on the training stability under an aggressive, warmup-free learning rate to study gradient stability.
